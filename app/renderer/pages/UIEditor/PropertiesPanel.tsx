...
// @ts-ignore
import style from './PropertiesPanel.css';
import {useInput} from '../../hooks';
import {changeBackground, changeBorder, changeShadow, changeText} from '../../actions';
...
function BackgroundProperties(props: IBackgroundPropertiesProps) {
  ...
  const handleChangeColor = (e: any) => {
    ...
    dispatch(changeBackground(e.target.value, props.background.opacity));
  };
  const handleChangeOpacity = (e: any) => {
    setOpacity(+e.target.value);
    dispatch(changeBackground(props.background.fill, +e.target.value));
  };
...
function BorderProperties(props: IBorderPropertiesProps) {
  ...
  const handleChangeColor = (e: any) => {
    setColor(e.target.value);
    dispatch(changeBorder(e.target.value, width, radius));
  };

  const handleChangeWidth = (e: any) => {
    setWidth(e.target.value);
    dispatch(changeBorder(color, +e.target.value, radius));
  };

  const handleChangeRadius = (e: any) => {
    setRadius(e.target.value);
    dispatch(changeBorder(color, width, + e.target.value));
  };
  return (
...
function ShadowProperties(props: IShadowPropertiesProps) {
  ...
  const handleChangeOffsetX = (e: any) => {
    ...
    setOffsetX(value);
    dispatch(changeShadow(color, blur, value, offsetY));
  };

  const handleChangeOffsetY = (e: any) => {
    const value = +e.target.value;
    setOffsetY(value);
    dispatch(changeShadow(color, blur, offsetX, value));
  };

  const handleChangeBlur = (e: any) => {
    const value = +e.target.value;
    setBlur(value);
    dispatch(changeShadow(color, value, offsetX, offsetY));
  };

  const handleChangeColor = (e: any) => {
    const value = e.target.value;
    setColor(value);
    dispatch(changeShadow(value, blur, offsetX, offsetY));
  };
...
function TextProperties(props: ITextPropertiesProps) {
  ...
  const handleChangeText = (e: any) => {
    const value = e.target.value;
    setText(value);
    dispatch(changeText(color, value));
  };

  const handleChangeColor = (e: any) => {
    const value = e.target.value;
    setColor(value);
    dispatch(changeText(value, text));
  };
...
export default function PropertiesPanel(props: IPropertiesPanelProps) {
  const cpnState = useSelector((state: IStoreState) => state.currentComponent);
